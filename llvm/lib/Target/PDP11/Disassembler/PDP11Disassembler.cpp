...
static MCDisassembler *createPDP11Disassembler(const Target & /*T*/,
                                               const MCSubtargetInfo &STI,
                                               MCContext &Ctx) {
  return new PDP11Disassembler(STI, Ctx);
}
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializePDP11Disassembler() {
  // Register the disassembler
  TargetRegistry::RegisterMCDisassembler(getThePDP11Target(),
                                         createPDP11Disassembler);
}

PDP11Disassembler::PDP11Disassembler(const MCSubtargetInfo &STI, MCContext &Ctx)
    : MCDisassembler(STI, Ctx) {}

#include "PDP11GenDisassemblerTables.inc"

DecodeStatus
PDP11Disassembler::getInstruction(MCInst &Instr, uint64_t &Size,
  ...
  return status == MCDisassembler::DecodeStatus::Success
             ? status
             : MCDisassembler::DecodeStatus::Fail;
}
